...
[`plotGauss.py <python/plotGauss.py>`_]
...
.. figure:: http://www.ecmwf.int/newsevents/training/rcourse_notes/DATA_ASSIMILATION/ASSIM_CONCEPTS/Assim_concepts243.gif
    :align: center

The figure above, from Bouttier and Courtier (1999) illustrates how such a strong constraint 4DVar works. 
...
.. plot::
    ...
    for sd in [1e-2,2e-2,4e-2]:
        ...
        y = np.array(F[N/2])[0]
        plt.plot(y/y.max(),label='SD: %f'%sd)
...
We can of course apply higher order derivative constraints and these are sometime appropriate (see Twomey, 2002). A second-order difference constraint is equivalent to (weakly) constraining the slope of the state trajectory to be constanti (a first-order process model).

.. figure:: http://www2.geog.ucl.ac.uk/~plewis/eoldas/_images/example1plot53.png
    :width: 90%
...
At heart, an update of the state :math:`x` is proposed, :math:`x'`, then a metric is calculated which weighs the likihood of the new state relative to the old state. If the new state is an improvement, then it is selected as an update. If it is not (the metric :math:`a` is less than 1), then a probability of :math:`a` is assigned to taking this new step.

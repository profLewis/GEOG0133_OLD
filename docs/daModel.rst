...
We first recall the canopy model that we developed in a previous exercise, based around the photosynthesis in Jules. This is available via the files: `canopyModel.py <python/daHard.py>`_, `modellingPractical.py <python/modellingPractical>`_, and `photJules.py <python/photJules.py>`_.
